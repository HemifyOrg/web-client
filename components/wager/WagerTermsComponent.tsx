...
import { SelectedTermType, WagerTermType } from "@/utils/types";
...
const TermComponent = ({
  ...
  setSelectedWagerTerm: React.Dispatch<
    React.SetStateAction<SelectedTermType | null>
  >;
}) => {
  return (
          ...
          <button
            ...
            className={`bg-gray-200 rounded-lg border-2 ${
              ...
              selectedWagerTerm?.option?.id === index
                ? "border-themeColor"
                : "border-transparent"
            } transition-all flex flex-col justify-center items-center xs:p-5 p-2 w-full h-full`}
          >
            <span
              className={`${
...
const WagerTermsComponent = ({
  ...
  return (
    <div className="w-full mt-5 flex flex-col gap-6 overflow-hidden select-none">
      <FilterMenuComponent
        tabs={tabs}
