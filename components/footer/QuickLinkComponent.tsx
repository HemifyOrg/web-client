      ...
      >
        {link.subLinks.map((link, index) => (
          <Link
            key={index}
            href={"#" + link.href}
            className="md:text-lg text-base group font-light flex md:gap-4 gap-2 no-underline hover:border-transparent border-transparent text-gray-100 hover:text-gray-200"
          >
            <span className="border-b border-transparent group-hover:border-[#d2b37d]">
              {link.title}
            </span>
            {link.isComingSoon && (
              <span className="text-xs border-b border-transparent group-hover:border- font-light bg-[#938558] rounded-lg py-1 px-3 text-gray-100">
                Coming Soon
              </span>
            )}
          </Link>
